...
| P-37 Tooltip-as-discovery (React portal) nie eksploatowane | 🟢 zamknięte (Sesja B verified) | **Sesja B verified Filipem 2026-05-15 20:20** — wszystkie 5 testowanych przycisków Notion Mail (Compose/Archive/Close sidebar/Reply/Forward) emit'ują toast via L0.3 po hoverze. `TooltipObserver` polluje pozycję kursora (200ms), gdy stabilny ≥350ms walk'uje drzewo AX frontmost app szukając floating AXGroup (40-500×16-100 px, ≤350px od kursora) z 2 AXStaticText: badge (1-8 znaków + obsługa `+` separator typu `⌘+\\`) + nazwa akcji (3-80 znaków po privacy filter). Hit-test pod kursorem zwraca rect przycisku (nie tooltipa). Zapis do `~/Library/Application Support/SFlow/discovered/{bundleId}.jsonl`. Nowa warstwa `L0.3` w `ClickWatcher` query'uje store po cursor-rect na początku każdego klika. **Sesja C (backend `/v1/discovered` crowdsource) pending — sub-cel 1.15 część 2.** |
