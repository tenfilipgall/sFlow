...
| P-37 Tooltip-as-discovery (React portal) nie eksploatowane | 🟡 w trakcie | Sesja B (2026-05-15) DONE — czeka na manual verify. `TooltipObserver` polluje pozycję kursora (200ms), gdy stabilny ≥350ms walk'uje drzewo AX frontmost app szukając floating AXGroup (40-500×16-100 px, ≤350px od kursora) z 2 AXStaticText: badge (1-5 znaków, parsowane przez `TooltipShortcutParser`) + nazwa akcji (3-80 znaków po privacy filter). Zapis do `~/Library/Application Support/SFlow/discovered/{bundleId}.jsonl`. Nowa warstwa `L0.3` w `ClickWatcher` query'uje store po cursor-rect na początku każdego klika. **Otwarte pytanie:** czy Chromium w Notion Mail w ogóle eksponuje tooltipe do AX — Sesja C (backend crowdsource) czeka na to potwierdzenie. |
