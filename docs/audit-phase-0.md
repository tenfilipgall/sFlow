...
| P-41 Right-click / context menu nie monitorowany | ✅ zamknięte 2026-05-17 | **NAPRAWIONE (U-2).** `CGEventTap` mask rozszerzony o `rightMouseDown`; po right-clicku `RightClickMenuHarvester` skanuje AX tree (depth ≤4) szukając `AXMenu`, iteruje wszystkie `AXMenuItem` dzieci, czyta `kAXMenuItemCmdChar` + `kAXMenuItemCmdModifiers` + `kAXMenuItemCmdVirtualKey`, parsuje modifier bitfield (Cmd implied unless NoCommand bit set; +Shift/Option/Control) → zapisuje `(action, keys, rect)` do `DiscoveredStore` z `source: "rightclick_menu"`. Następny left-click w menu trafia L0.3 lookup → toast. `DiscoveredStore.lookup` ma relaxed rect filter (400×200) dla menu entries (item rows ~300×29 są >200×200 tooltip threshold). **Pokrywa wszystkie apki naraz, zero per-app pracy.** Pliki: `SFlow/RightClickMenuHarvester.swift` (nowy 160 linii) + `ClickWatcher.handleRightMouseDown` + `DiscoveredStore.source` field. 14 testów passing. Adresuje Sub-cel 1.18 (G-1) w Fazie 1.5. |
