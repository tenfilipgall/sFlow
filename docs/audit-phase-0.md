...
| P-37 Tooltip-as-discovery (React portal) nie eksploatowane | ⬜ otwarte | Sesja 2026-05-15: klatki ze screen-graba pokazują że Notion Mail (i podobne React apki) renderują własne tooltipy z parą `(nazwa akcji, skrót)` — np. "Compose a new email / C", "Close sidebar / ⌘\\". Tooltipy są portalowane do `document.body` → w drzewie AX są **rodzeństwem** AXWebArea, nie dziećmi przycisku. To gotowy, sparowany sygnał na tacy. **Plan:** `TooltipObserver` (sub-cel 1.15) pasywnie wykrywa pojawienie się takich AXGroup-tooltipów + filtr prywatności + zapis lokalny do `discovered/{bundleId}.jsonl` + crowdsource przez `/v1/discovered` backend → bundled rule updates dla wszystkich userów. Sesje B+C. |
