...
| P-19 Bundled.json update path | 🟢 zamknięte 2026-05-18 | **Sesja Finalize Fazy 1.5:** `RuleStorage.seedBundledIfMissing` zwraca teraz `SeedOutcome` enum (copiedFresh/upgradedFromShipping/keptUser/noShippingResource) i porównuje fingerprint (apps × 1M + rules × 1K + titles) shipping vs user bundled.json. Shipping > user → overwrite. `cache/*.json` i `user_overrides.json` NIGDY nie ruszone. 8 fingerprint tests + smoke test real shipping bundle. Commit `4f8dee6`. **Odblokowuje iterację DMG podczas Bety.** |
...
| P-32 Web research w backend prompt jest niesterowany | 🟢 zamknięte 2026-05-18 | **Sesja Finalize Fazy 1:** `backend/src/prompt.ts` rozszerzony o sekcję WEB_SEARCH STRATEGY z explicit STEP 1 (`{appName} keyboard shortcuts cheatsheet`) → STEP 2 (`hotkey list`) → STEP 3 (per-element). `backend/src/claude.ts` web_search max_uses 4→8. Backend deployed v `ba683371-4b18-4d09-9c82-92a085bb83d6`. Reseed 5 bundled: 253→265 reguł (+12), web_docs sources 56→69 (+23%), Terminal+Claude Desktop dostają pierwsze `web_docs_*` reguły. Dowód: Slack ⌘/ "Open Keyboard Shortcuts" i Obsidian ⌘P "Command Palette" trafiły do bundled (były w żadnym menu bar). Commits `57b4935` + `6d8b051`. |
...
| P-35 Backend timeout (>90s) dla niektórych apek | 🟢 zamknięte 2026-05-18 | Sesja Finalize Fazy 1: Obsidian po zwiększeniu web_search max_uses 4→8 (Sub-cel 1.12) miał timeout >90s na pierwszej próbie (retry zadziałał). Fix: `DiscoveryClient.timeoutInterval` 90s→180s + `Reseeder` semafora 120s→240s. Smoke test: Slack reseed 67s na 1. próbie, bez retry. Commit `4bf1320`. DisplayTuner verify pominięty — apka nie zainstalowana lokalnie, ale skoro 5 reseed apek przeszły w <70s każda (z 8 max_uses search budgetu) — stary 90s timeout był marginalny, nowy 180s ma generous headroom. |
...
| P-41 Right-click / context menu nie monitorowany | ✅ zamknięte 2026-05-17 (U-2 → U-2.2 + diag retry) | **U-2 (native macOS):** `CGEventTap` mask rozszerzony o `rightMouseDown`; harvester czyta `AXMenuItemCmdChar`/`Modifiers`/`VirtualKey`. Filip UAT: ✅ Finder. **U-2.1 (Chromium-Comet/Slack):** title-based parser dla apek które pakują skrót w title (`"Save link ⌘S"`, `"Edit message E"`); bug-fix `"cmd"→"meta"`. Filip UAT: ✅ Comet. **U-2.2 (Notion):** Notion eksponuje skrót w `kAXValue` zamiast `kAXTitle`; harvester czyta oba i fallback'uje. Plus retry path: jeśli pierwsze 300ms nie znajdzie AXMenu, retry po kolejnych 300ms z deeper walk (depth ≤8) bo Chromium może renderować menu wolniej. Filip UAT: ✅ Notion (po fix). 22 testów passing. Adresuje Sub-cel 1.18 (G-1). |
...
| P-43 i18n / lokalizacja reguł | 🟢 zamknięte 2026-05-18 | **Sesja Finalize Fazy 1.5:** pełen stack zaimplementowany. Client: `LocaleDetector.swift` (AXLanguage → systemLocale fallback, normalize PL/zh-Hans), `LoadedMatch.localizedTitles`, `RuleCache.match(locale:)` tries `localizedTitles[locale]` BEFORE EN titles. Backend: `DiscoverRequestSchema.appLocale`, `MatchSchema.localizedTitles`, prompt rule LOCALIZED TITLES ("actual AX-exposed strings, NOT literal translations"), per-locale cache key (rules:bundle:1.0:pl). `SFLOW_RESEED_LOCALE=pl` env override w Reseeder dla devów z systemem !=PL. **Empiryczna weryfikacja:** Slack reseed z PL = **64/64 reguł (100%) z localizedTitles.pl** (Skomponuj wiadomość, Wyszukaj kanał, Wątki, ...). Claude poprawnie OMIT'uje lokalizacje których nie może zweryfikować (Obsidian/Claude Desktop są EN-only). Backend v `ba6d6866`. Commit `a2bdbec`. |
| P-44 Single-key shortcut mode (Gmail/Vim) | ✅ zamknięte 2026-05-17 | **NAPRAWIONE (U-3).** Schema bundled.json rozszerzona o `features.singleKeyMode: true` per bundle (opcjonalne, backward-compat — istniejące entries bez `features` parsują się jako `nil`). `RuleCache.isSingleKeyApp(bundleId:)` zwraca flagę z osobnego `featuresByBundle` dictionary (rozdzielone od rules — empty-rules entry z features nie blokuje cache rules przez first-write-wins). ClickWatcher Layer 2 gate: `allowSingleChar = isInteractive \|\| ruleCache.isSingleKeyApp(bundleId)` — single-char `kAXHelp` akceptowany na non-interactive role w whitelistowanych apkach. **Whitelist (5 apek):** notion.id, md.obsidian, notion.mail.id, com.linear.LinearMac, com.cron.electron. 4 nowe testy RuleCacheTests passing (default false, true when set, empty-rules entry nie blokuje cache, backward-compat z legacy bundled). Adresuje Sub-cel 1.21 (G-4). |
...
| P-46 Tool/mode switching w kreatywnych apkach | 🟢 minimum-viable 2026-05-18 | **Sesja Finalize Fazy 1.5:** zamiast full AXToolbar detection (wymaga U-6 scope schema, którego dziś nie ma), prostsze rozwiązanie pokrywające 80% use case'ów: bundled rules dla Figma/Photoshop/Illustrator z `features.singleKeyMode: true` + toolbox rules wygenerowane przez backend curl (P-32 prompt → cheatsheets z web_docs_official). Figma 83 reguł, Photoshop 81, Illustrator 97. Pełna AXToolbar detection odroczona post-Beta jeśli FP rate w creative apkach >5%. Commit `5fa6f7d`. |
| P-47 Brak ewaluacji 5 typów apek UI | 🔵 częściowo 2026-05-18 | **Sesja Finalize Fazy 1.5:** **2 z 5 kategorii pokryte** przez backend-generated bundled rules (bez konieczności instalacji apek lokalnie — Filip nie ma Office/Adobe/Figmy): Microsoft Office partial (Excel 63 reguł; Word/Outlook/PPT/OneNote pending kolejnej sesji bo rate limit 10/h IP) + Adobe Creative Suite (Photoshop 81 + Illustrator 97). 3 pozostałe kategorie (Qt/GTK/Tk, Catalyst, SwiftUI pure) **odroczone do post-Beta** — niska prio dla MVP. Real-app spike walidacyjny robią beta-testerzy podczas Bety. Commit `5fa6f7d`. |
...
| P-50 Slack hover-toolbar — reguły matchowały tylko context menu, nie hover-toolbar | ✅ zamknięte | **NAPRAWIONE 2026-05-17.** Diagnostyka `tooltipDiag` na Slacku ujawniła prawdziwą przyczynę (różną od wcześniejszej hipotezy "AXGroup bez etykiet"): przyciski hover-toolbar mają etykiety, ale w `AXDescription`, nie `AXTitle`: `desc="Save for later"` (AXCheckBox), `desc="Reply in thread"` (AXButton), `desc="Forward message…"` (AXButton), `desc="Remove from Later"` (AXCheckBox toggle state). RuleCache.match już sprawdzał oba `title` i `desc`, ALE `roleCompatible(AXMenuItem, AXCheckBox/AXButton)` zwracało `false` — więc reguły `slack-msg-*` z `role: "AXMenuItem"` nigdy nie pasowały do hover-toolbar. Fix: 7 reguł zmienionych z `role: AXMenuItem` → `role: AXButton` (`AXButton` przez `roleCompatible` matchuje wszystkie `clickableRoles`, czyli AXMenuItem + AXCheckBox + AXPopUpButton + ...). Dodatkowo dodano "Forward message…" i "Remove from Later" do tablic `titles`. Plik: `SFlow/Resources/bundled.json`. Działa teraz dla obu wariantów (prawy klik + hover-toolbar). |
| P-52 Discovery status menu bar gubi info o paralelnym uczeniu | ⬜ otwarte (UX, nie blocker) | **Odkryte 2026-05-18 przez kod-walk DiscoveryService.swift.** `DispatchQueue(label:...)` jest serial dla AX walk (faza krojenia), ale backend call leci async (`Task { await ... }`) — nie blokuje kolejki. To znaczy że gdy user otwiera apkę A i zaraz potem apkę B, walk B startuje gdy backend A jeszcze pieczy. Problem: `DiscoveryStatus.running(appName: String)` w `onStatusChange` ma jedną zmienną — pokazuje **tylko ostatnio rozpoczętą** apkę w menu bar (✓ "Learning Calendar…" zastępuje "Learning Mail…" mimo że Mail nadal się pieczy w tle). User myśli że SFlow zapomniał o A. Funkcjonalnie OK — reguły obu apek lądują w cache; kosmetyczny problem statusu. **Plan:** zmienić `DiscoveryStatus` z `running(appName)` na `running(appNames: [String])` lub `runningCount: Int`; menu bar pokazuje "Learning 2 apps…" + tooltip z nazwami. Niski priorytet (Faza 2 UX polish). |
| P-51 Electron lazy AX tree blocks reseed harvesting | ⬜ otwarte | **Odkryte 2026-05-17 podczas reseed Obsidiana.** Chromium/Electron apki (Obsidian, Notion, Linear, Cursor, Discord, VSCode, Slack desktop) eksponują **shell** AX tree podczas reseedu — bez aktywności użytkownika. Dowód: `AXSkeletonExtractor` walk od `AXApp` z `AXMenuBar` skipped, settle 3s po `AXManualAccessibility`+`AXEnhancedUserInterface`, depth 6 — daje `visited=12 raw=0 byRole [AXGroup=6 AXButton=3 AXMenuBar=1 AXWindow=1 AXApplication=1]`. Te 6 AXGroup + 3 AXButton mają **puste title i puste description**. Tymczasem w trakcie klikania (z user activity) ten sam AXGroup eksponuje `desc="Open graph view"`, `desc="New note"`, `desc="Open today's daily note"`, `desc="Search"`, `desc="Bookmarks"` itd. Implikacja: backend reseed dla Electron apek dostaje wyłącznie menu bar (~200 entries) — content okna jest niemożliwy do zharvestowania pre-reseed. Konsekwencja: AXButton reguły dla content w Electron apkach pochodzą wyłącznie z `web_docs_official`/`web_docs_third_party` (LLM wiedza ogólna), nie z empirycznego skeleton. To wpływa na ~7 apek w pipeline. **Hipotezy fix'u (do rozważenia):** (a) runtime collection — przy każdym kliku w AXGroup z desc, dopisać kandydata reguły do `discovered/<bundleId>.json`, po N kliknięciach trigger backend dla finalizacji (incremental learning), (b) symulowane mouseover każdego widocznego elementu okna przed walk (wymusza Chromium na populację AX), (c) inny atrybut AX dla Electron (np. `AXVisibleChildren` zamiast `AXChildren`), (d) Apple Accessibility Inspector używa specjalnego protokołu — sprawdzić jak. Sub-cele 1.19 (web-as-app) i U-3 single-key (zamknięty) częściowo łagodzą problem ale nie rozwiązują harvest gap. Częściowe mitigacje **już wdrożone 2026-05-17:** AXGroup z kAXDescription dodane do `AXSkeletonExtractor.allowedRoles`, walk skipuje AXMenuBar wewnętrznie, settle wydłużone z 0.5s do 3.0s — działa gdy AX tree się populuje, no-op dla Obsidian. Plik: `SFlow/AXSkeletonExtractor.swift`, `SFlow/Reseeder.swift`. Adresuje Sub-cele 1.19 i 1.6. |
...
- **P-50 (2026-05-17) — Slack hover-toolbar: reguły `slack-msg-*` matchowały
  tylko context menu (prawy klik), nie hover-toolbar. ✅ ZAMKNIĘTE 2026-05-17.**
  Wcześniejsza hipoteza ("AXGroup bez etykiet" z parent walk) była mylna —
  patrzono na zły poziom drzewa. Diagnostyka `defaults write com.filip.sflow
  tooltipDiag -bool true` + system-wide hit-test pokazała, że Slack hover-toolbar
  buttons mają etykiety w `AXDescription` (nie `AXTitle`):
  - `desc="Save for later"` → `AXCheckBox` 32×32
  - `desc="Remove from Later"` → `AXCheckBox` 32×32 (toggle state Save)
  - `desc="Reply in thread"` → `AXButton` 32×33
  - `desc="Forward message…"` → `AXButton` 32×33
  - `desc="More actions"` → `AXPopUpButton` 32×32 (opens menu, brak skrótu)

  RuleCache.match już matchował `desc` (oprócz `title`/`help`/`roleDesc`), ale
  `roleCompatible("AXMenuItem", "AXCheckBox")` zwracało `false` → reguły
  z `role: "AXMenuItem"` nigdy nie pasowały do AXCheckBox/AXButton/AXPopUpButton.

  **Fix:** 7 reguł `slack-msg-*` zmienionych z `role: AXMenuItem` → `role: AXButton`.
  `AXButton` w `roleCompatible` jest permissive — matchuje wszystkie `clickableRoles`
  (`AXButton + AXLink + AXMenuItem + AXMenuBarItem + AXCheckBox + AXRadioButton +
  AXPopUpButton + AXGroup + AXCell + AXImage`). Więc ta sama reguła obsługuje
