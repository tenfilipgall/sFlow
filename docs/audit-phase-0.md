...
| P-41 Right-click / context menu nie monitorowany | ✅ zamknięte 2026-05-17 (U-2 + U-2.1) | **U-2 (native macOS):** `CGEventTap` mask rozszerzony o `rightMouseDown`; po right-clicku `RightClickMenuHarvester` skanuje AX tree (depth ≤4) szukając `AXMenu`, iteruje `AXMenuItem`, czyta `kAXMenuItemCmdChar`/`Modifiers`/`VirtualKey`, parsuje modifier bitfield → DiscoveredStore z `source: "rightclick_menu"`, relaxed rect filter (400×200). Filip UAT 2026-05-17: ✅ Finder działa. **U-2.1 fallback (Chromium):** UAT ujawnił że Notion/Comet używają emulated AXMenu który NIE eksponuje cmdChar — skrót jest w `title` (np. `"Save link ⌘S"`, `"Edit message E"`, `"Open in tab ⌘Click"`). Dodano `parseShortcutFromTitle` z 2 wzorcami: (1) trailing macOS-symbol + key, (2) slack-style trailing ` X`. Mouse modifiers (`⌘Click`/`⌥Click`/`⌃Drag`) odrzucone — nie do wciśnięcia z klawiatury. Plus bug-fix: konwencja `"cmd"` → `"meta"` (zgodne z KeySymbols + ShortcutRules). Pliki: `SFlow/RightClickMenuHarvester.swift` + `ClickWatcher.handleRightMouseDown` + `DiscoveredStore.source`. 22 testów passing (14 native + 8 title parser). Adresuje Sub-cel 1.18 (G-1). |
