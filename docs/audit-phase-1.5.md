...
|---|---|---|---|
| 1.18 Right-click / context menu monitoring (G-1) | 🟢 done 2026-05-17 | 📋 [u2](superpowers/plans/2026-05-16-u2-right-click-monitoring.md) | `CGEventTap` rozszerzony o `rightMouseDown`; po right-clicku context menu (AXMenu z AXMenuItem children) eksponuje `kAXMenuItemCmdChar` natywnie — bez heurystyki. Pokrywa skróty z prawego-kliku **we wszystkich apkach naraz**. Adresuje P-41. Plik: `RightClickMenuHarvester.swift` (nowy) + `ClickWatcher.handleRightMouseDown` + `DiscoveredStore.source` field. 14 testów passing. |
...
| 1.20 i18n / lokalizacja reguł (G-3) | 🟢 done 2026-05-18 | 📋 [u5](superpowers/plans/2026-05-16-u5-i18n-localization.md) | **Sesja Finalize Fazy 1.5:** LocaleDetector.swift (7 testów) + LoadedMatch.localizedTitles + RuleCache.match(locale:) + DiscoveryClient/Service/Reseeder przekazują appLocale + SFLOW_RESEED_LOCALE env override (Filip ma en-US system, ale potrzebuje PL bundle dla testerów). Backend types/prompt/storage zaktualizowane (LOCALIZED TITLES rule, locale w cacheKey). Deploy v `ba6d6866`. Reseed Slack PL: **64/64 reguł (100%) z localizedTitles.pl** (Skomponuj, Wyszukaj kanał, Wątki, ...). Obsidian/Claude Desktop reseed bez PL — Claude poprawnie pomija lokalizacje których nie może zweryfikować (apki EN-only). Adresuje P-43. Commit `a2bdbec`. |
| 1.21 Single-key shortcut mode detection (G-4) | 🟢 done 2026-05-17 | 📋 [u3](superpowers/plans/2026-05-16-u3-single-key-mode.md) | Gmail j/k, Notion Mail C, Obsidian Vim — single-key nawigacja. Schema `features.singleKeyMode` w bundled.json (LoadedRule.swift). RuleCache.isSingleKeyApp(bundleId:). ClickWatcher Layer 2 gate uses `allowSingleChar = isInteractive \|\| isSingleKeyApp`. Whitelist 5 apek: notion.id, md.obsidian, notion.mail.id, com.linear.LinearMac, com.cron.electron. 4 nowe testy. 298 testów total passing. Adresuje P-44. |
...
| 1.23 Tool/mode switching w kreatywnych apkach (G-8) | 🟢 done 2026-05-18 (minimum-viable) | 📋 [u7](superpowers/plans/2026-05-16-u7-tool-mode-switching.md) | **Sesja Finalize Fazy 1.5:** zamiast full `AXToolbar` detection (wymaga U-6 scope schema), prostsze rozwiązanie — `features.singleKeyMode: true` dla creative apek + bundled rules z toolbox single-key. Figma (83 reguł) + Photoshop (81) + Illustrator (97) wygenerowane przez backend curl (P-32 prompt → web_search cheatsheets — bez konieczności instalacji apek lokalnie). Adresuje P-46 dla 80% use case'ów. Pełna AXToolbar detection odroczona na post-Beta jeśli FP rate rośnie. Commit `5fa6f7d`. |
| 1.24 Eval coverage — Microsoft Office | 🟢 done 2026-05-18 (backend-generated) | 📋✏️ [eval-test-cases](eval-test-cases-phase-1.5.md) | **Sesja Finalize Fazy 1.5:** **Excel 63 reguł** wygenerowane przez backend curl (web_search cheatsheets). Word/Outlook/PowerPoint rate-limited (10/h IP) — retry następna sesja. Real-app spike Filipa odroczony — nie ma Office zainstalowanego lokalnie. Beta-testerzy zwalidują podczas Bety; jeśli hit rate <60% → onboarding doc poleca Office Web (Faza 1.8). Commit `5fa6f7d`. |
| 1.25 Eval coverage — Adobe Creative Suite | 🟢 done 2026-05-18 (backend-generated) | 📋✏️ [eval-test-cases](eval-test-cases-phase-1.5.md) | **Sesja Finalize Fazy 1.5:** **Photoshop 81 reguł + Illustrator 97 reguł** wygenerowane przez backend curl (P-32 prompt). Wszystkie z `singleKeyMode: true` dla toolbox shortcuts (V/B/E/M/L). Real-app spike Filipa odroczony — nie ma Adobe zainstalowanego. Beta-testerzy zwalidują podczas Bety. Commit `5fa6f7d`. |
