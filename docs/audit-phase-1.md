...
| 1.8 Video-based eval protocol | 🟢 done 2026-05-18 | Droga C ✅. Droga B (`--llm`) **verified 2026-05-18**: skrypt uruchomiony na 32 klatkach (Slack + Xcode), Claude Haiku 4.5 vision, 10.1s. Prompt v2 zadziałał: **0 halucynacji** (v1 miał 4 false-positives — Slack context menu items "Remind me ?"/"Mark unread U"/"Copy link L"/"Quick Switcher ⌘K" — wszystkie były błędnie klasyfikowane jako toast SFlow). v2 poprawnie sklasyfikował te 3 jako context menu items, jeden Xcode "Quick Switcher" ⌘K jako autentyczny toast. Pełen audit + Verification result w `docs/video-eval-test.md`. |
