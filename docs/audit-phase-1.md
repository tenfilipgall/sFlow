...
| 1.15 Passive tooltip observer + backend ingest (P-37) | 🟡 w trakcie | **Sesja B (2026-05-15) DONE — czeka na verify.** `TooltipObserver` z pollingiem kursora (200ms), `TooltipShortcutParser`, `DiscoveredStore` z lokalnym zapisem do `discovered/{bundleId}.jsonl` + lookup po cursor-rect. Nowa warstwa `L0.3 tooltipObserver` w `ClickWatcher` query'uje store na początku każdego klika. 33 nowe testy, 252 total passing. Sesja C (backend `/v1/discovered`) pending po manual verify że tooltipy Notion Maila są faktycznie eksponowane do drzewa AX. |
...
| **B** | Passive tooltip observer | 1.15 część 1 (P-37) — `TooltipObserver` + click-time L0.3 fallback + lokalny `discovered/{bundleId}.jsonl` + privacy filter | ~3h | 🟡 w trakcie | inline (czeka na manual verify) |
